...
> Note: compiling the framework entirely from source generates quite a large build cache (~2.4 GB). Be sure to have at least 5 GB available on your device before you start compiling to make sure your OS keeps functioning.
...
A typical Brane installation should at least require the command-line tool `brane` and the Brane instance itself (six Docker images). You can compile these with:
```bash
...
To work around this, you can also compile a 'development' instance (`dev` for short) that compiles it in debug mode and that cross-compiles the framework for a container rather than compiling it inside a container.

The difference is, though, that each image now only contains a script that runs the mounted, compiled binary that lives outside of the container. This makes them extremely non-portable; so please don't use this method of compilation for production environments.
...
> Note: installing the instance this way results in an _even larger_ build cache (~4.1 GB). To this end, make sure you have at least 7 GB available before compiling.
...
In this book, we will continue in the subsequent chapter to describe how to deploy the instance on a [Kubernetes]() cluster so it may run on more than one machine. This is useful for large-scale production environments 
